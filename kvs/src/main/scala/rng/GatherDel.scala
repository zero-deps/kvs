...
class GatherDel(client: ActorRef, t: FiniteDuration, prefList: Set[Node], k: Array[Byte], conf: RngConf) extends FSM[FsmState, Set[Node]] with ActorLogging {
...
object GatherDel {
  def props(client: ActorRef, t: FiniteDuration, prefList: Set[Node], k: Array[Byte], conf: RngConf): Props = Props(new GatherDel(client, t, prefList, k, conf))
}
