...
object WriteStore {
  def props(db: RocksDB, hashing: Hashing)(using CanEqual[None.type, Option[BucketInfo]], CanEqual[None.type, Option[Data]]): Props = Props(WriteStore(db, hashing))
}

class WriteStore(db: RocksDB, hashing: Hashing)(using CanEqual[None.type, Option[BucketInfo]], CanEqual[None.type, Option[Data]]) extends Actor with ActorLogging {
  ...
  def doDelete(key: Array[Byte]): String = {
    ...
    b_info.foreach{ b_info =>
      ...
      val keys = b_info.keys.filterNot(java.util.Arrays.equals(_, key))
      withBatch(batch => {
