...
object MergeOps {
  def forDump(xs: Vector[KeyBucketData])(using CanEqual[None.type, Option[KeyBucketData]]): Vector[KeyBucketData] =
    @tailrec
  ...
  def forRepl(xs: Vector[KeyBucketData])(using CanEqual[None.type, Option[KeyBucketData]]): Vector[KeyBucketData] =
    @tailrec
    def loop(xs: Vector[KeyBucketData], acc: HashMap[Bytes, KeyBucketData]): Vector[KeyBucketData] =
      xs match
        case Vector() => acc.values.toVector
  ...
  def forGatherGet(xs: Vector[AddrOfData])(using CanEqual[None.type, Option[Data]]): (Option[Data], HashSet[Node]) =
    @tailrec
  ...
  def forPut(stored: Option[Data], received: Data)(using CanEqual[None.type, Option[Data]]): Option[Data] =
    stored match
      case None => 
        Some(received)
      case Some(stored) =>
