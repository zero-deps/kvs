...
import data.{Data, BucketInfo, StoreKey, DataKey, BucketInfoKey}, data.codec.*, data.keycodec.*, model.{DumpGetBucketData, DumpBucketData}, model.{ReplBucketsVc, ReplGetBucketIfNew, ReplBucketUpToDate, ReplNewerBucketData, KeyBucketData}, model.{StoreGet, StoreGetAck}, ReplicationSupervisor.ReplGetBucketsVc

object ReadonlyStore {
  def props(db: RocksDB, hashing: Hashing)(using CanEqual[None.type, Option[BucketInfo]]): Props = Props(ReadonlyStore(db, hashing))
}

class ReadonlyStore(db: RocksDB, hashing: Hashing)(using CanEqual[None.type, Option[BucketInfo]]) extends Actor with ActorLogging {
