...
object KvsFeed {
  ...
  def manualFeed[Fid: MessageCodec, Key: MessageCodec, A: MessageCodec](fidPrefix: Array[Byte]): Manual[Fid, Key, A] =
    new Manual[Fid, Key, A] {
      def fdKey(fid: Fid): UIO[FdKey] = UIO(FdKey(encode(Named(fidPrefix, fid))))
      def elKey(key: Key): UIO[ElKey] = UIO(ElKey(encode(key)))
  ...
  def incrementFeed[Fid: MessageCodec, Key, A: MessageCodec](fidPrefix: Array[Byte], from: Key => Array[Byte], to: Array[Byte] => Key): Increment[Fid, Key, A] =
  ...
  private object Shard {
    sealed trait Msg
    case class Add(fid: FdKey, data: Array[Byte]) extends Msg
    case class Put(fid: FdKey, id: ElKey, data: Array[Byte]) extends Msg
    ...
    case class PutBulk(fid: FdKey, a: Vector[(ElKey,Array[Byte])]) extends Msg
