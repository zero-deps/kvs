...
object KvsDirectory:
  ...
  val layer: ZLayer[Dba & DirName, Nothing, KvsDirectory] =
    ZLayer(
      for
        dba <- ZIO.service[Dba]
        dirname <- ZIO.service[DirName]
      yield
        given dbaEff: DbaEff = DbaEff(dba)
        given CanEqual[BrokenFile.type, BrokenFile.type | FileNotExists.type | dbaEff.Err] = CanEqual.derived
        given CanEqual[BrokenListing.type, BrokenListing.type | FileNotExists.type | dbaEff.Err] = CanEqual.derived
        given CanEqual[FileExists.type, FileExists.type | dbaEff.Err] = CanEqual.derived
        given CanEqual[FileNotExists.type, FileNotExists.type | dbaEff.Err |  BrokenListing.type] = CanEqual.derived
        given ce1: CanEqual[None.type, Option[Array[Byte]]] = CanEqual.derived
        given ce2: CanEqual[None.type, Option[Either[dbaEff.Err | FileNotExists.type | BrokenFile.type | FileExists.type, Unit]]] = CanEqual.derived
        given ce3: CanEqual[None.type, Option[Fd]] = CanEqual.derived
        KvsDirectory(dirname)
    )
...
class KvsDirectory(
  ...
  CanEqual[BrokenFile.type, BrokenFile.type | FileNotExists.type | dba.Err]
, CanEqual[BrokenListing.type, BrokenListing.type | FileNotExists.type | dba.Err]
, CanEqual[FileExists.type, FileExists.type | dba.Err]
, CanEqual[FileNotExists.type, FileNotExists.type | BrokenListing.type | dba.Err]
...
, CanEqual[None.type, Option[Fd]]
) extends BaseDirectory(NoLockFactory.INSTANCE):
