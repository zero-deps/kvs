...
object Fd {
  val empty = new Fd(head=None, length=0, removed=0, maxid=ElKey(Array.emptyByteArray))
}
 ...
 */
object feed {
  implicit val ekc: MessageCodec[ElKey] = caseCodecAuto
   ...
   */
  def put(key: EnKey, data: Array[Byte])(implicit dba: Dba): IO[Err, Unit] = {
    for {
      x  <- _get(key)
      y  <- x.cata(y =>
              for {
  ...
  def get(key: EnKey)(implicit dba: Dba): IO[Err, Option[Array[Byte]]] = {
    _get(key).map(_.map(_.data))
  }

  def apply(key: EnKey)(implicit dba: Dba): IO[Err, Array[Byte]] = {
   ...
   */
  def add(fid: FdKey, data: Array[Byte])(implicit dba: Dba): IO[Err, ElKey] = {
    for {
      fd1 <- meta.get(fid)
   ...
   * Adds the entry to the container. Creates the container if it's absent.
   */
  def putIfAbsent(key: EnKey, data: Array[Byte])(implicit dba: Dba): IO[Err, Unit] = {
    for {
      en1 <- _get(key)
  ...
  def head(fid: FdKey)(implicit dba: Dba): IO[Err, Option[(ElKey, Array[Byte])]] = {
    all(fid).runHead
  }
  ...
  def all(fid: FdKey)(implicit dba: Dba): Stream[Err, (ElKey, Array[Byte])] = {
    entries_live(fid).map{ case (id,en) => id->en.data }
  }

  /* all data without removed from specified key */
  def all(fid: FdKey, start: ElKey)(implicit dba: Dba): Stream[Err, (ElKey, Array[Byte])] = {
  ...
  def fix(fid: FdKey, fd: Fd)(implicit dba: Dba): IO[Err, Unit] = {
    for {
      x  <- entries(fid).fold((0L, 0L, ElKey(Array.emptyByteArray))){
              case ((l,r,m),(id,en)) =>
  ...
  private def max(x: ElKey, y: ElKey): ElKey = {
    ...
    if (Arrays.compare(x.bytes, y.bytes) > 0) x else y
  }
}
