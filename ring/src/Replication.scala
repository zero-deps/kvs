...
object ReplicationSupervisor {
  ...
  def props(buckets: SortedMap[Bucket, PreferenceList], conf: Conf): Props = {
...
class ReplicationSupervisor(initialState: State, conf: Conf) extends FSM[FsmState, State] with ActorLogging {
...
object ReplicationWorker {
  case class ReplState(prefList: PreferenceList, info: Vector[Vector[KeyBucketData]], vc: VectorClock)

  def props(b: Bucket, prefList: PreferenceList, vc: VectorClock, conf: Conf): Props = Props(new ReplicationWorker(b, prefList, vc, conf))
}

class ReplicationWorker(b: Bucket, _prefList: PreferenceList, _vc: VectorClock, conf: Conf) extends FSM[FsmState, ReplState] with ActorLogging {
