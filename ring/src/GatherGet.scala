...
class GatherGet(client: ActorRef, t: FiniteDuration, M: Int, @unused R: Int, k: Array[Byte]) extends FSM[FsmState, DataCollection] with ActorLogging {
...
object GatherGet {
  def props(client: ActorRef, t: FiniteDuration, M: Int, R: Int, k: Array[Byte]): Props = Props(new GatherGet(client, t, M, R, k))

  type AddrOfData = (Option[Data], Node)

  case class DataCollection(perNode: Vector[AddrOfData], nodes: Int)
}
