...
object Search:
  trait Service:
    val dirname: String
    def run[A](q: Query, `doc->a`: Document => A, limit: Int=10)(using ClassTag[A]): ZStream[Any, Throwable, A]
    def index[R, E, A](xs: ZStream[R, E, A], `a->doc`: A => Document): ZIO[R, E | Throwable, Unit]
  end Service

  val live: ZLayer[Has[KvsDirectory], Throwable, Has[Service]] = ZLayer.fromService(dir =>
    new Service {
      val dirname = dir.dir

      def run[A](q: Query, `doc->a`: Document => A, limit: Int)(using ClassTag[A]): ZStream[Any, Throwable, A] =
        for
          reader <- ZStream.managed(ZManaged.fromAutoCloseable(ZIO.effect(DirectoryReader.open(dir))))
          searcher <- ZStream.fromEffect(ZIO.effect(IndexSearcher(reader)))
          xs <- ZStream.fromIterable(searcher.search(q, limit).scoreDocs.map(x => `doc->a`(searcher.doc(x.doc))))
        yield xs
