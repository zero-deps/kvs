...
class FeedSpec extends TestKit(ActorSystem("FeedSpec"))
  ...
  "feed" - {
    "no 1" - {
      ...
      "should save e1" in {
        val saved = kvs.prepend(EnKey(fid, key1), data(1))
        saved shouldBe ().right
        fd.length(fid) shouldBe 1L.right
      }

      "should save e2" in {
        val saved = kvs.prepend(EnKey(fid, key2), data(2))
        saved shouldBe ().right
        fd.length(fid) shouldBe 2L.right
      }

      "should get e1 and e2 from feed" in {
        fd.length(fid) shouldBe 2L.right 
        kvs.all(fid).map(_.toList) shouldBe List((key2 -> data(2)).right, (key1 -> data(1)).right).right
      }

      "should save entry(3)" in {
        val saved = kvs.prepend(EnKey(fid, key3), data(3))
        saved shouldBe ().right
        fd.length(fid) shouldBe 3L.right
      }

      "should not save entry(2) again" in {
        kvs.prepend(EnKey(fid, key2), data(2)).fold(identity, identity) shouldBe EntryExists(EnKey(fid, key2))
        fd.length(fid) shouldBe 3L.right
      }

      "should get 3 values from feed" in {
        kvs.all(fid).map(_.toList) shouldBe List((key3 -> data(3)).right, (key2 -> data(2)).right, (key1 -> data(1)).right).right
      }

      "should remove unexisting entry from feed without error" in {
        kvs.remove(EnKey(fid, keyN(5))) shouldBe false.right
      }

      "should remove entry(2)" in {
        val deleted = kvs.remove(EnKey(fid, key2))
        deleted shouldBe true.right
      }
      ...
      "should get 2 values from feed" in {
        ...
        stream.map(_.toList) shouldBe List((key3 -> data(3)).right, (key1 -> data(1)).right).right
      }

      "should remove entry(1) from feed" in {
        val deleted = kvs.remove(EnKey(fid, key1))
        deleted shouldBe true.right
      }
      ...
      "should get 1 values from feed" in {
        ...
        val stream = kvs.all(fid)
        stream.map(_.toList) shouldBe List((key3 -> data(3)).right).right
      }

      "should remove entry(3) from feed" in {
        val deleted = kvs.remove(EnKey(fid, key3))
        deleted shouldBe true.right
      }

      "should be empty at the end test" - {
        "length is 0" in { fd.length(fid) shouldBe 0L.right }
        "all is empty" in { kvs.all(fid) shouldBe LazyList.empty.right }
        "delete fd is ok" in { fd.delete(fid) shouldBe Right(()) }
        "delete is idempotent" in { fd.delete(fid) shouldBe Right(()) }
    ...
    "no 2" - {
      ...
      "insert id 5" in {
        val s = kvs.prepend(EnKey(fid, keyN(5)), data(0))
      ...
      "insert id 'a'" in {
        val s = kvs.prepend(EnKey(fid, keyN('a')), data(0))
        s.isRight shouldBe true
      }
      ...
      "insert id 3" in {
        val s = kvs.prepend(EnKey(fid, key3), data(0))
        s.isRight shouldBe true
      }
    ...
    "no 3" - {
      ...
      "insert nine entries" in {
        0.to(9).foreach(i => kvs.prepend(EnKey(fid, keyN(i.toByte)), data(0)))
      }
      "maxid is 9" in {
        fd.get(fid).map(_.map(_.maxid)) shouldBe keyN(9).some.right
      }
      "remove entries" in {
        Seq(0, 1, 4, 5, 8, 9).foreach(i => kvs.remove(EnKey(fid, keyN(i.toByte))))
      }
      ...
      "maxid is reverted to 7" in {
        fd.get(fid).map(_.map(_.maxid)) shouldBe keyN(7).some.right
      }
